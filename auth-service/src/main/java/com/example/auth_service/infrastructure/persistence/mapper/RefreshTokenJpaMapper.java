...
import com.example.auth_service.domain.models.token.vo.GenerationNumber;
import com.example.auth_service.domain.models.token.vo.RevokedReason;
import com.example.auth_service.domain.models.token.vo.TokenExpiry;
...
public class RefreshTokenJpaMapper {
    ...
    public RefreshTokenJpaEntity toJpaEntity(RefreshToken token, UserJpaEntity userRef, boolean isNew) {
        ...
        return RefreshTokenJpaEntity.builder()
                ...
                .user(userRef)
                .expiryDate(token.getExpiry().expiry())
                .revoked(token.isRevoke())
                .revokedAt(token.getRevokedAt())
                .family(token.getFamily().value())
                .generation(token.getGeneration().value())
                .revokedReason(token.getReason() != null ? token.getReason().reason() : null)
